...
class Command extends BaseCommand_1.default {
    constructor(client, handler) {
        ...
        this.run = (M, { joined }) => __awaiter(this, void 0, void 0, function* () {
            ...
            console.log(chitoge);
            const { data } = yield axios_1.default.get(`https://api-xcoders.xyz/api/info/trend/google?country=${chitoge}&apikey=Zl0clXuAbx`);
            const buffer = yield request_1.default.buffer(data.result.data[0].thumbnail).catch((e) => {
                return void M.reply(e.message);
            });
            while (true) {
                try {
                    M.reply(buffer || '✖ An error occurred. Please try again later', baileys_1.MessageType.image, undefined, undefined, `${data.result.massage}\n *(1)* ${data.result.data[0].title}\n ${data.result.data[0].url}\n\n*(2)* ${data.result.data[1].title}\n ${data.result.data[1].url}\n\n*(3)* ${data.result.data[2].title}\n ${data.result.data[2].url} `, undefined).catch((e) => {
